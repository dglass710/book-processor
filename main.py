...
def prompt_for_chapter_descriptions(chapter_count, titles):
    ...
    for i, title in enumerate(titles):
        if title == "Front Matter":
            ...
            print("Description for Back Matter (e.g., 'Index, Bibliography, Appendices')")
        elif title is not None:
            print(f"Description for {title}")
        else:
            print(f"Description for Chapter {i + 1}")
        description = input(f"Description for {title if title is not None else f'Chapter {i + 1}'}: ").strip()
...
def main():
    ...
    if file_type not in ["epub", "mobi"]:
        ...
        ocr = OCRProcessor()
        success, message, text_files = ocr.process_images_parallel(image_files, dirs["text"], language="eng", prefix="page")
    ...
    chapters = organizer.parse_chapter_locations(chapter_pages, chapter_titles, last_chapter_end + 1, page_offset)
    ...
    success, message, processed_chapters = organizer.process_chapters(chapters, dirs["text"], dirs["chapters"])
    ...
    success, message, output_files = combined_organizer.create_combined_files(groups, processed_chapters, dirs["chapters"], dirs["combined"])
    ...
    # Create index file
    print("\nCreating index file")
    success, message, index_path = combined_organizer.create_index_file(groups, processed_chapters, dirs["combined"], book_title)
